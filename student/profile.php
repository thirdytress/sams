<?php
// profile.php — NU SAMS | Student Portal — My Profile
...
$student_year = '';
$date_joined = '';
$profile_status = 'Active Student Assistant';

$stmt = $mysqli->prepare('
    SELECT full_name, email, contact_number, course, year_level, work_location, application_status, created_at 
...
if ($stmt) {
    ...
    if ($stmt->execute()) {
        ...
        if ($row = $result->fetch_assoc()) {
            $student_name = htmlspecialchars($row['full_name'] ?? $student_name);
            $student_email = htmlspecialchars($row['email'] ?? '');
            $student_contact = htmlspecialchars($row['contact_number'] ?? '');
            $student_course = htmlspecialchars($row['course'] ?? '');
            $student_year = htmlspecialchars($row['year_level'] ?? '');
            $date_joined = $row['created_at'] ? date('F Y', strtotime($row['created_at'])) : '';
            $profile_status = $row['application_status'] === 'approved' ? 'Active Student Assistant' : ucfirst(str_replace('_', ' ', $row['application_status'] ?? 'pending'));
        }
        $result->free();
    }
    $stmt->close();
...
$stmt_hours = $mysqli->prepare('
    SELECT SUM(TIME_TO_SEC(TIMEDIFF(check_out_time, check_in_time)) / 3600) as total_hours
    FROM attendance
...
if ($stmt_hours) {
    ...
    if ($stmt_hours->execute()) {
        ...
        if ($row_hours = $result_hours->fetch_assoc()) {
            $total_hours = intval($row_hours['total_hours'] ?? 0);
        }
        $result_hours->free();
    }
    $stmt_hours->close();
...
// Calculate duties done (from attendance records with status 'present')
$duties_done = 0;
$stmt_duties = $mysqli->prepare('
    SELECT COUNT(*) as duties_count
    FROM attendance
    WHERE student_id = ? AND status = "present"
');

if ($stmt_duties) {
    $stmt_duties->bind_param('s', $student_id);
    if ($stmt_duties->execute()) {
        $result_duties = $stmt_duties->get_result();
        if ($row_duties = $result_duties->fetch_assoc()) {
            $duties_done = intval($row_duties['duties_count'] ?? 0);
        }
        $result_duties->free();
    }
    $stmt_duties->close();
...
$week_hours = 0;
$stmt_week = $mysqli->prepare('
    SELECT SUM(TIME_TO_SEC(TIMEDIFF(check_out_time, check_in_time)) / 3600) as week_hours
    FROM attendance
    WHERE student_id = ? AND check_out_time IS NOT NULL AND YEARWEEK(check_in_time) = YEARWEEK(NOW())
');

if ($stmt_week) {
    $stmt_week->bind_param('s', $student_id);
    if ($stmt_week->execute()) {
        $result_week = $stmt_week->get_result();
        if ($row_week = $result_week->fetch_assoc()) {
            $week_hours = intval($row_week['week_hours'] ?? 0);
        }
        $result_week->free();
    }
    $stmt_week->close();
...
$month_hours = 0;
$stmt_month = $mysqli->prepare('
    SELECT SUM(TIME_TO_SEC(TIMEDIFF(check_out_time, check_in_time)) / 3600) as month_hours
    FROM attendance
    WHERE student_id = ? AND check_out_time IS NOT NULL AND MONTH(check_in_time) = MONTH(NOW()) AND YEAR(check_in_time) = YEAR(NOW())
');

if ($stmt_month) {
    $stmt_month->bind_param('s', $student_id);
    if ($stmt_month->execute()) {
        $result_month = $stmt_month->get_result();
        if ($row_month = $result_month->fetch_assoc()) {
            $month_hours = intval($row_month['month_hours'] ?? 0);
        }
        $result_month->free();
    }
    $stmt_month->close();
...
if ($stmt_rate) {
    $stmt_rate->bind_param('s', $student_id);
    if ($stmt_rate->execute()) {
        $result_rate = $stmt_rate->get_result();
        if ($row_rate = $result_rate->fetch_assoc()) {
            if ($row_rate['total_count'] > 0) {
                $attendance_rate = intval(($row_rate['present_count'] / $row_rate['total_count']) * 100);
            }
        }
        $result_rate->free();
    }
...
<head>
    ...
    <title>My Profile | NU SAMS</title>
    <style>
        /* ============================================================
            ...
            <ul class="nav__list">
                ...
                <li class="nav__item">
                    <a href="profile.php" class="nav__link nav__link--active" aria-current="page">
                        <span class="nav__icon" aria-hidden="true">
                            <svg viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <circle cx="10" cy="7" r="4" stroke="#003087" stroke-width="1.5"/>
                                <path d="M2 17c0-3.314 3.582-6 8-6s8 2.686 8 6" stroke="#003087" stroke-width="1.5" stroke-linecap="round"/>
                            </svg>
                        </span>
        ...
        <section class="page-content" aria-label="My Profile">
            ...
            <div class="profile-grid">
                ...
                <div class="avatar-card" role="region" aria-label="Profile overview">
                    ...
                    <!-- Name & ID -->
                    <h2 class="avatar-card__name"><?php echo htmlspecialchars($student_name); ?></h2>
                    <p class="avatar-card__id"><?php echo htmlspecialchars($student_id); ?></p>

                    <!-- Status Badge -->
                    <span class="avatar-card__status-badge" role="status"><?php echo htmlspecialchars($profile_status); ?></span>

                    <!-- Stats -->
                    <div class="avatar-card__stats">
                        <div class="avatar-stat avatar-stat--blue">
                            <span class="avatar-stat__value"><?php echo $total_hours; ?></span>
                            <span class="avatar-stat__label">Total Hours</span>
                        </div>
                        <div class="avatar-stat avatar-stat--gold">
                            <span class="avatar-stat__value"><?php echo $duties_done; ?></span>
                            <span class="avatar-stat__label">Duties Done</span>
                        </div>
                    </div>
                    ...
                    <div class="avatar-card__actions">
                        ...
                        <button class="btn-change-pin" type="button">
                            <svg viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="right-col">
                    ...
                    <div class="info-card" role="region" aria-label="Personal Information">
                        ...
                        <div class="fields-grid">
                            ...
                            <div class="field">
                                ...
                                <span class="field__value"><?php echo htmlspecialchars($student_name); ?></span>
                            </div>
                            <!-- Student ID -->
                            <div class="field">
                                <span class="field__label">Student ID</span>
                                <span class="field__value"><?php echo htmlspecialchars($student_id); ?></span>
                            ...
                            <div class="field">
                                ...
                                <span class="field__value"><?php echo htmlspecialchars($student_email ?: 'Not provided'); ?></span>
                            </div>
                            <!-- Contact -->
                            <div class="field">
                                <span class="field__label">
                                ...
                                <span class="field__value"><?php echo htmlspecialchars($student_contact ?: 'Not provided'); ?></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="info-card" role="region" aria-label="Academic Information">
                        ...
                        <div class="fields-grid">
                            ...
                            <div class="field">
                                ...
                                <span class="field__value"><?php echo htmlspecialchars($student_course ?: 'Not specified'); ?></span>
                            </div>
                            <!-- Year -->
                            <div class="field">
                                <span class="field__label">Year Level</span>
                                <span class="field__value"><?php echo htmlspecialchars($student_year ?: 'Not specified'); ?></span>
                            </div>
                            <!-- Date Joined -->
                            <div class="field">
                                <span class="field__label">
                                ...
                                <span class="field__value"><?php echo htmlspecialchars($date_joined ?: 'Recently joined'); ?></span>
                            </div>
                            <!-- Status -->
                            <div class="field">
                                <span class="field__label">
                                ...
                                <span class="field__status-pill field__status-pill--green" role="status"><?php echo htmlspecialchars($profile_status); ?></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="perf-card" role="region" aria-label="Performance Summary">
                        ...
                        <div class="perf-stats">
                            <div class="perf-stat">
                                <span class="perf-stat__label">This Week</span>
                                <span class="perf-stat__value"><?php echo $week_hours; ?> hrs</span>
                            </div>
                            <div class="perf-stat">
                                <span class="perf-stat__label">This Month</span>
                                <span class="perf-stat__value"><?php echo $month_hours; ?> hrs</span>
                            </div>
                            <div class="perf-stat">
                                <span class="perf-stat__label">Attendance Rate</span>
                                <span class="perf-stat__value"><?php echo $attendance_rate; ?>%</span>
                            </div>
                        </div>
                        ...
                        <div class="achievement">
                            ...
                            <div class="achievement__row">
                                ...
                                <div class="achievement__info">
                                    <span class="achievement__title"><?php echo $attendance_rate >= 95 ? 'Perfect Attendance!' : 'Great Work!'; ?></span>
                                    <span class="achievement__subtitle"><?php echo $attendance_rate >= 95 ? 'No missed duties this month 🎉' : 'Keep up the excellent work!'; ?></span>
                                </div>
                            </div>
                        </div>
                    </div>
