<?php
// attendance.php — NU SAMS | Student Portal — Attendance QR Generator
...
function normalize_work_location_label(string $value): string {
    $value = trim($value);
    if ($value === '') {
    ...
    return $lookup[$key] ?? ucwords(str_replace(['_', '-'], ' ', $value));
}

function format_attendance_time(?string $value): string {
...
function attendance_generate_otp_code(int $length = 6): string {
    $min = (int) pow(10, $length - 1);
    $max = (int) pow(10, $length) - 1;
    return (string) random_int($min, $max);
}

function attendance_mask_email(string $email): string {
    $email = trim($email);
...
function attendance_build_otp_email_html(string $toName, string $otpCode, string $actionLabel, string $location): string {
    $safeName = htmlspecialchars($toName !== '' ? $toName : 'Student');
    $safeOtp = htmlspecialchars($otpCode);
    $safeAction = htmlspecialchars($actionLabel);
    $safeLocation = htmlspecialchars($location !== '' ? $location : 'Assigned Office');
    ...
    return '<div style="margin:0;padding:24px;background:#f3f7ff;font-family:Inter,Segoe UI,Arial,sans-serif;color:#111827;">'
        ...
        . '<div style="font-size:22px;font-weight:800;letter-spacing:.3px;">NU SAMS Attendance OTP</div>'
        ...
        . '<p style="margin:0 0 16px 0;font-size:14px;line-height:1.6;color:#334155;">Use this OTP to confirm your ' . $safeAction . ' request for <strong>' . $safeLocation . '</strong>.</p>'
        ...
        . '<p style="margin:0;font-size:13px;color:#64748b;">If you did not request this code, you can ignore this email.</p>'
        . '</td></tr>'
        . '</table>'
        . '</div>';
}

function attendance_send_otp_email(string $toEmail, string $toName, string $otpCode, string $actionLabel, string $location, string &$error): bool {
    ...
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = 'NU SAMS Attendance OTP - ' . $actionLabel;
        $mail->Body = attendance_build_otp_email_html($toName, $otpCode, $actionLabel, $location);
        $mail->AltBody = "Your NU SAMS attendance OTP for {$actionLabel} is {$otpCode}. It expires in 10 minutes.";

        $mail->send();
        return true;
    } catch (\PHPMailer\PHPMailer\Exception $ex) {
        $error = 'Unable to send OTP email. Mailer error: ' . $mail->ErrorInfo;
        if (stripos($mail->ErrorInfo, 'Could not authenticate') !== false) {
            $error .= ' For Gmail, use an App Password and enable 2-Step Verification.';
        }
        return false;
    }
}

function attendance_has_open_log_today(mysqli $mysqli, string $student_id): bool {
    if ($stmt = $mysqli->prepare("SELECT 1 FROM attendance WHERE student_id = ? AND DATE(check_in_time) = CURDATE() AND check_out_time IS NULL LIMIT 1")) {
        $stmt->bind_param('s', $student_id);
        if ($stmt->execute()) {
            $result = $stmt->get_result();
            $hasOpen = $result ? ($result->num_rows > 0) : false;
            $result && $result->free();
            $stmt->close();
            return $hasOpen;
        }
...
function attendance_record_check_in(mysqli $mysqli, string $student_id, string $student_name, string $student_location): bool {
    if (attendance_has_open_log_today($mysqli, $student_id)) {
        return false;
    }

    if ($insert = $mysqli->prepare('INSERT INTO attendance (student_id, student_name, location, status, check_in_time, check_out_time) VALUES (?, ?, ?, ?, NOW(), NULL)')) {
        $status = 'present';
        $insert->bind_param('ssss', $student_id, $student_name, $student_location, $status);
        $ok = $insert->execute();
        $insert->close();
        return $ok;
    }
...
function attendance_record_check_out(mysqli $mysqli, string $student_id): bool {
    if ($update = $mysqli->prepare("UPDATE attendance SET check_out_time = NOW() WHERE student_id = ? AND DATE(check_in_time) = CURDATE() AND check_out_time IS NULL ORDER BY check_in_time DESC LIMIT 1")) {
...
$student_email = '';
$student_location = 'Assigned Office';
$attendance_notice = '';
$attendance_notice_type = 'success';
$otp_notice = '';
$otp_notice_type = 'success';
$otp_max_attempts = 5;
$otp_resend_cooldown = 60;
$otp_expiry_seconds = 600;
$attendance_mode = strtolower(trim((string) ($_GET['mode'] ?? $_SESSION['attendance_mode'] ?? 'otp')));
if (!in_array($attendance_mode, ['qr', 'otp'], true)) {
    $attendance_mode = 'otp';
}
$_SESSION['attendance_mode'] = $attendance_mode;
...
if ($has_status_column) {
    $st = $mysqli->prepare('SELECT full_name, email, work_location, application_status FROM student_applications WHERE student_id = ? ORDER BY created_at DESC LIMIT 1');
    if ($st) {
        $st->bind_param('s', $student_id);
        if ($st->execute()) {
            $resSt = $st->get_result();
            if ($appRow = $resSt ? $resSt->fetch_assoc() : null) {
                $student_name = (string) ($appRow['full_name'] ?? $student_name);
                $student_email = (string) ($appRow['email'] ?? '');
                $student_location = normalize_work_location_label((string) ($appRow['work_location'] ?? ''));
...
$attendance_pending = $_SESSION['attendance_otp_pending'] ?? null;
if (!is_array($attendance_pending) || (string) ($attendance_pending['student_id'] ?? '') !== $student_id) {
    $attendance_pending = null;
    unset($_SESSION['attendance_otp_pending']);
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $attendance_stage = strtolower(trim((string) ($_POST['attendance_stage'] ?? 'request')));
    $attendance_action = strtolower(trim((string) ($_POST['attendance_action'] ?? '')));
    $otp_input = trim((string) ($_POST['otp_code'] ?? ''));

    if ($attendance_mode !== 'otp') {
        $attendance_notice = 'Switch to OTP mode to request attendance verification.';
        $attendance_notice_type = 'error';
    } elseif ($attendance_stage === 'request') {
        if (!in_array($attendance_action, ['check_in', 'check_out'], true)) {
            $attendance_notice = 'Please choose Check In or Check Out first.';
            $attendance_notice_type = 'error';
        } elseif ($student_email === '') {
            $attendance_notice = 'No email address is available for your account.';
            $attendance_notice_type = 'error';
        } elseif ($attendance_action === 'check_in' && attendance_has_open_log_today($mysqli, $student_id)) {
            $attendance_notice = 'You are already checked in today.';
            $attendance_notice_type = 'error';
        } elseif ($attendance_action === 'check_out' && !attendance_has_open_log_today($mysqli, $student_id)) {
            $attendance_notice = 'No active check in found for today.';
            $attendance_notice_type = 'error';
        } else {
            $otp_code = attendance_generate_otp_code();
            $_SESSION['attendance_otp_pending'] = [
            ...
            $mail_error = '';
            if (attendance_send_otp_email($student_email, $student_name, $otp_code, attendance_action_label($attendance_action), $student_location, $mail_error)) {
                $attendance_notice = 'OTP sent to ' . attendance_mask_email($student_email) . '. Enter it below to confirm ' . attendance_action_label($attendance_action) . '.';
                $attendance_notice_type = 'success';
                $otp_notice = 'OTP sent successfully.';
                $otp_notice_type = 'success';
            } else {
                unset($_SESSION['attendance_otp_pending']);
                $attendance_notice = $mail_error !== '' ? $mail_error : 'Unable to send OTP right now.';
                $attendance_notice_type = 'error';
            }
        }
    } elseif ($attendance_stage === 'resend') {
        if (!$attendance_pending) {
            $attendance_notice = 'There is no pending OTP to resend.';
            $attendance_notice_type = 'error';
        } elseif (time() < (int) ($attendance_pending['last_sent_at'] ?? 0) + $otp_resend_cooldown) {
            $attendance_notice = 'Please wait before requesting another OTP.';
            $attendance_notice_type = 'error';
        } else {
            $attendance_action = (string) ($attendance_pending['action'] ?? 'check_in');
            $otp_code = attendance_generate_otp_code();
            $_SESSION['attendance_otp_pending']['otp_hash'] = password_hash($otp_code, PASSWORD_DEFAULT);
            $_SESSION['attendance_otp_pending']['expires_at'] = time() + $otp_expiry_seconds;
            $_SESSION['attendance_otp_pending']['sent_at'] = time();
            $_SESSION['attendance_otp_pending']['last_sent_at'] = time();
            $_SESSION['attendance_otp_pending']['attempts'] = 0;

            $mail_error = '';
            if (attendance_send_otp_email($student_email, $student_name, $otp_code, attendance_action_label($attendance_action), $student_location, $mail_error)) {
                $attendance_notice = 'OTP re-sent to ' . attendance_mask_email($student_email) . '.';
                $attendance_notice_type = 'success';
            } else {
                $attendance_notice = $mail_error !== '' ? $mail_error : 'Unable to resend OTP right now.';
                $attendance_notice_type = 'error';
            }
        }
    } elseif ($attendance_stage === 'verify') {
        $attendance_pending = $_SESSION['attendance_otp_pending'] ?? null;
        if (!is_array($attendance_pending) || (string) ($attendance_pending['student_id'] ?? '') !== $student_id) {
            $attendance_notice = 'No pending OTP found. Please request a new code.';
            $attendance_notice_type = 'error';
        } elseif ((int) ($attendance_pending['expires_at'] ?? 0) < time()) {
            unset($_SESSION['attendance_otp_pending']);
            $attendance_notice = 'Your OTP has expired. Please request a new one.';
            $attendance_notice_type = 'error';
        } else {
            $attempts = (int) ($attendance_pending['attempts'] ?? 0);
            if ($attempts >= $otp_max_attempts) {
                unset($_SESSION['attendance_otp_pending']);
                $attendance_notice = 'Too many failed attempts. Please request a new OTP.';
                $attendance_notice_type = 'error';
            } elseif ($otp_input === '') {
                $attendance_notice = 'Enter the OTP sent to your email.';
                $attendance_notice_type = 'error';
            } elseif (!password_verify($otp_input, (string) ($attendance_pending['otp_hash'] ?? ''))) {
                $_SESSION['attendance_otp_pending']['attempts'] = $attempts + 1;
                $remainingAttempts = max(0, $otp_max_attempts - ($attempts + 1));
                $attendance_notice = 'Invalid OTP. ' . $remainingAttempts . ' attempt' . ($remainingAttempts === 1 ? '' : 's') . ' left.';
                $attendance_notice_type = 'error';
            } else {
                $pendingAction = (string) ($attendance_pending['action'] ?? 'check_in');
                $ok = false;

                if ($pendingAction === 'check_in') {
                    $ok = attendance_record_check_in($mysqli, $student_id, $student_name, $student_location);
                } elseif ($pendingAction === 'check_out') {
                    $ok = attendance_record_check_out($mysqli, $student_id);
                }

                if ($ok) {
                    unset($_SESSION['attendance_otp_pending']);
                    $attendance_pending = null;
                    $attendance_notice = attendance_action_label($pendingAction) . ' recorded successfully.';
                    $attendance_notice_type = 'success';
                } else {
                    $attendance_notice = 'Could not record your attendance. Please try again.';
                    $attendance_notice_type = 'error';
                }
            }
        }
...
$otp_pending_action = (string) ($attendance_pending['action'] ?? '');
$otp_pending_action_label = attendance_action_label($otp_pending_action);
$otp_pending_email = (string) ($attendance_pending['email'] ?? '');
$otp_pending_expires_at = (int) ($attendance_pending['expires_at'] ?? 0);
$otp_pending_sent_at = (int) ($attendance_pending['sent_at'] ?? 0);
$otp_attempts_used = (int) ($attendance_pending['attempts'] ?? 0);
$otp_attempts_remaining = max(0, $otp_max_attempts - $otp_attempts_used);
$otp_expires_in = max(0, $otp_pending_expires_at - time());
$otp_resend_available_in = max(0, ($otp_pending_sent_at + $otp_resend_cooldown) - time());
...
$valSelect = $has_validation_column ? ", validation_status" : "";
$recent_logs = [];
if ($stmt = $mysqli->prepare("SELECT location, check_in_time, check_out_time{$valSelect} FROM attendance WHERE student_id = ? ORDER BY check_in_time DESC LIMIT 5")) {
    $stmt->bind_param('s', $student_id);
    if ($stmt->execute()) {
        $result = $stmt->get_result();
        while ($row = $result ? $result->fetch_assoc() : null) {
            ...
            $isOpen = empty($row['check_out_time']);
            $recent_logs[] = [
                'location' => (string) ($row['location'] ?? $student_location),
                'time_in' => !empty($row['check_in_time']) ? date('M j, g:i A', strtotime((string) $row['check_in_time'])) : '-',
                'time_out' => !empty($row['check_out_time']) ? date('M j, g:i A', strtotime((string) $row['check_out_time'])) : '-',
                'status' => $isOpen ? 'In Progress' : 'Completed',
                'status_class' => $isOpen ? 'att-status--active' : 'att-status--present',
                'validation' => (string) ($row['validation_status'] ?? 'pending'),
            ];
        }
        $result && $result->free();
    }
...
$today_time_in = '-';
$today_time_out = '-';
$today_status_label = 'No log yet';
$today_action = 'in';

if ($stmt = $mysqli->prepare('SELECT check_in_time, check_out_time FROM attendance WHERE student_id = ? AND DATE(check_in_time) = CURDATE() ORDER BY check_in_time DESC LIMIT 1')) {
    $stmt->bind_param('s', $student_id);
    if ($stmt->execute()) {
        $result = $stmt->get_result();
        if ($row = $result ? $result->fetch_assoc() : null) {
            $today_time_in = format_attendance_time($row['check_in_time'] ?? null);
            $today_time_out = format_attendance_time($row['check_out_time'] ?? null);
            $hasOpenLog = empty($row['check_out_time']);
            $today_status_label = $hasOpenLog ? 'Checked In' : 'Checked Out';
            $today_action = $hasOpenLog ? 'out' : 'in';
        }
        $result && $result->free();
    }
    $stmt->close();
...
$attendance_display_mode_label = $attendance_mode === 'otp' ? '🔢 OTP Mode' : '📱 QR Code';
$attendance_mode_label = $attendance_pending ? ('🔐 ' . $otp_pending_action_label . ' OTP') : ($today_action === 'out' ? '🚪 Check Out' : '✅ Check In');
$attendance_mode_qr_label = $attendance_pending ? ('🔐 ' . strtoupper($otp_pending_action_label)) : ($today_action === 'out' ? '🚪 CHECK OUT' : '✅ CHECK IN');
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .otp-form__input {
            height: 48px;
            border: 1px solid var(--clr-border-input, #D1D5DC);
            border-radius: var(--radius-md);
            padding: 0 16px;
            font-size: var(--fs-base);
        ...
        <section class="page-content" aria-label="Attendance QR Generator">
            ...
            <div class="panel-grid">
                ...
                <div class="qr-card">
                    ...
                    <?php if ($attendance_notice !== ''): ?>
                        <div class="attendance-notice attendance-notice--<?php echo htmlspecialchars($attendance_notice_type); ?>" role="status">
                            <?php echo htmlspecialchars($attendance_notice); ?>
                        </div>
                    <?php endif; ?>

                    <!-- Mode Toggle -->
                    <div class="mode-toggle" role="group" aria-label="Display mode">
                        <button class="btn-mode <?php echo $attendance_mode === 'qr' ? 'btn-mode--active' : 'btn-mode--inactive'; ?>" id="btnQrMode" type="button" aria-pressed="<?php echo $attendance_mode === 'qr' ? 'true' : 'false'; ?>" data-mode="qr">
                            <svg viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <button class="btn-mode <?php echo $attendance_mode === 'otp' ? 'btn-mode--active' : 'btn-mode--inactive'; ?>" id="btnOtpMode" type="button" aria-pressed="<?php echo $attendance_mode === 'otp' ? 'true' : 'false'; ?>" data-mode="otp">
                            <svg viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <?php if ($attendance_mode === 'otp'): ?>
                        <form class="checkin-toggle" method="post" role="group" aria-label="Attendance type">
                            <input type="hidden" name="attendance_stage" value="request" />
                            <button class="btn-checkin <?php echo $today_action === 'in' ? 'btn-checkin--active' : 'btn-checkin--inactive'; ?>" id="btnCheckIn" type="submit" name="attendance_action" value="check_in" aria-pressed="<?php echo $today_action === 'in' ? 'true' : 'false'; ?>">Check In</button>
                            <button class="btn-checkin <?php echo $today_action === 'out' ? 'btn-checkin--active' : 'btn-checkin--inactive'; ?>" id="btnCheckOut" type="submit" name="attendance_action" value="check_out" aria-pressed="<?php echo $today_action === 'out' ? 'true' : 'false'; ?>">Check Out</button>
                        </form>
                    <?php else: ?>
                        <div class="attendance-notice attendance-notice--success">Switch to OTP mode to request a time in or time out code.</div>
                    <?php endif; ?>
                    ...
                    <div class="expiry-banner" aria-live="polite" aria-label="Code expiry countdown">
                        <div class="expiry-banner__left">
                            ...
                            <div class="expiry-banner__info">
                                <span class="expiry-banner__label"><?php echo $attendance_mode === 'otp' ? 'OTP expires in:' : 'Code expires in:'; ?></span>
                                <span class="expiry-banner__value" id="countdownDisplay" data-expire-seconds="<?php echo (int) $otp_expires_in; ?>" data-resend-seconds="<?php echo (int) $otp_resend_available_in; ?>"><?php echo $attendance_mode === 'otp' && $attendance_pending ? (int) $otp_expires_in . ' seconds' : '5 seconds'; ?></span>
                            </div>
                        </div>
                    ...
                    <?php if ($attendance_mode === 'otp'): ?>
                        <div class="otp-panel">
                            <div class="otp-panel__header">
                                <span class="otp-panel__chip">OTP Security</span>
                                <div class="otp-panel__title">Email verification is required</div>
                                <div class="otp-panel__sub">
                                    <?php if ($attendance_pending): ?>
                                        OTP was sent to <?php echo htmlspecialchars(attendance_mask_email($otp_pending_email)); ?> for <?php echo htmlspecialchars($otp_pending_action_label); ?>.
                                    <?php else: ?>
                                        Request an OTP for check in or check out, then verify it below.
                                    <?php endif; ?>
                                </div>
                            </div>

                            <?php if ($attendance_pending): ?>
                                <form class="otp-form" method="post" id="otpVerifyForm">
                                    <input type="hidden" name="attendance_stage" value="verify" />
                                    <label class="otp-panel__sub" for="otpCode">Enter the 6-digit OTP</label>
                                    <input class="otp-form__input" id="otpCode" name="otp_code" type="text" inputmode="numeric" autocomplete="one-time-code" maxlength="6" placeholder="000000" />
                                    <div class="otp-form__row">
                                        <button class="otp-button otp-button--primary" type="submit">Verify OTP</button>
                                    </div>
                                </form>

                                <form class="otp-form" method="post" id="otpResendForm">
                                    <input type="hidden" name="attendance_stage" value="resend" />
                                    <div class="otp-form__row">
                                        <button class="otp-button otp-button--ghost" type="submit" <?php echo $otp_resend_available_in > 0 ? 'disabled' : ''; ?>>Resend OTP</button>
                                    </div>
                                    <div class="otp-panel__hint">
                                        Attempts left: <?php echo (int) $otp_attempts_remaining; ?> | Resend available in: <span id="resendCountdown"><?php echo (int) $otp_resend_available_in; ?>s</span>
                                    </div>
                                </form>
                            <?php endif; ?>
                        </div>
                    ...
                    <div class="qr-panel-wrap">
                        <div class="qr-display" aria-label="QR code for attendance check-in">
                            ...
                            <div class="qr-display__info">
                                <p class="qr-display__label" id="qrModeLabel"><?php echo htmlspecialchars($attendance_mode_qr_label); ?></p>
                                <p class="qr-display__sublabel"><?php echo $attendance_mode === 'otp' ? 'Use the emailed OTP to confirm your attendance' : 'Scan this QR code to log attendance'; ?></p>
                            </div>
                        </div>

                        <!-- Code ID -->
                        <div class="code-id-strip">
                            <span class="code-id-strip__label"><?php echo $attendance_mode === 'otp' ? 'OTP DELIVERY:' : 'CURRENT CODE ID:'; ?></span>
                            <span class="code-id-strip__value" id="codeIdDisplay"><?php echo $attendance_mode === 'otp' ? ($attendance_pending ? 'OTP sent to ' . attendance_mask_email($otp_pending_email) : 'Request an OTP to continue') : ('SAMS-' . time() . '-' . strtoupper(substr(bin2hex(random_bytes(3)), 0, 6))); ?></span>
                        </div>
                    </div>
                    ...
                    <div class="info-card" role="region" aria-label="Current Status">
                        ...
                        <div class="status-rows">
                            <div class="status-row">
                                ...
                                <span class="status-row__value" id="statusDisplayMode"><?php echo htmlspecialchars($attendance_display_mode_label); ?></span>
                            </div>
                            <div class="status-row">
                                <span class="status-row__label">Attendance Type</span>
                                <span class="status-row__value" id="statusAttType"><?php echo htmlspecialchars($attendance_mode_label); ?></span>
                            </div>
                            <div class="status-row">
                                ...
                                <div class="status-row__value-with-icon">
                                    ...
                                    <span class="status-row__value-text"><?php echo htmlspecialchars($student_location); ?></span>
                                </div>
                            </div>
                            <div class="status-row">
                                ...
                                <div class="status-row__value-with-icon">
                                    ...
                                    <span class="status-row__value-text" id="currentTime"><?php echo date('h:i A'); ?></span>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="recent-logs-list">
                                <?php foreach ($recent_logs as $log): ?>
                                    <div class="recent-log">
                                        <div class="recent-log__top">
                                            <div class="recent-log__location"><?php echo htmlspecialchars($log['location']); ?></div>
                                            <span class="att-status <?php echo htmlspecialchars($log['status_class']); ?>"><?php echo htmlspecialchars($log['status']); ?></span>
                                        </div>
                                        <div class="recent-log__meta">
                                            <span>Time In</span>
                                            <strong><?php echo htmlspecialchars($log['time_in']); ?></strong>
                                        </div>
                                        <div class="recent-log__meta">
                                            <span>Time Out</span>
                                            <strong><?php echo htmlspecialchars($log['time_out']); ?></strong>
                                        </div>
                                        <?php if ($log['time_out'] !== '-'): ?>
                                        <div class="recent-log__meta">
                                            <span>Admin Validation</span>
                                            <strong>
                                                <?php if ($log['validation'] === 'accepted'): ?>
                                                    <span style="color:#008236;">Accepted</span>
                                                <?php elseif ($log['validation'] === 'rejected'): ?>
                                                    <span style="color:#b42318;">Rejected</span>
                                                <?php else: ?>
                                                    <span style="color:#F54900;">Pending</span>
                                                <?php endif; ?>
                                            </strong>
                                        </div>
                                        <?php endif; ?>
...
(function () {
    ...
    var attendanceMode = <?php echo json_encode($attendance_mode); ?>;
    var otpPending = <?php echo $attendance_pending ? 'true' : 'false'; ?>;
    var otpRemainingSeconds = <?php echo (int) $otp_expires_in; ?>;
    var otpResendRemainingSeconds = <?php echo (int) $otp_resend_available_in; ?>;
    ...
    function refreshCode() {
        if (attendanceMode === 'otp' && otpPending) {
            if (otpResendForm && otpResendRemainingSeconds <= 0) {
    ...
    function updateCountdown() {
        if (countdownEl) {
            if (attendanceMode === 'otp' && otpPending) {
                countdownEl.textContent = otpRemainingSeconds + ' second' + (otpRemainingSeconds !== 1 ? 's' : '');
            } else {
        ...
        if (resendCountdown) {
            resendCountdown.textContent = otpResendRemainingSeconds + 's';
        }
    }
