<?php
// status.php – NU SAMS Application Status Page
session_start();
require_once __DIR__ . '/db.php';

$student_id_param = trim((string) ($_GET['student_id'] ?? $_SESSION['student_id'] ?? ''));
if ($student_id_param === '') {
...
$stmt = $mysqli->prepare("SELECT full_name, student_id, course, year_level, created_at{$statusSelect} FROM student_applications WHERE student_id = ? ORDER BY created_at DESC LIMIT 1");
...
if ($row = $res->fetch_assoc()) {
    ...
    $student_id = (string) ($row['student_id'] ?? $student_id_param);
    $course = (string) ($row['course'] ?? '');
    $year_level = (string) ($row['year_level'] ?? '');
    $createdAt = (string) ($row['created_at'] ?? '');
    $status = strtoupper(trim((string) ($row['application_status'] ?? 'pending')));
    $date_submitted = $createdAt !== '' ? date('F j, Y', strtotime($createdAt)) : '';
} else {
    header('Location: login.php');
    exit;
}
...
$state_title = '⏳ Application Under Review';
$state_sub = 'Your application is currently being reviewed by Miss Zai. This typically takes 1-3 business days.';

if ($status === 'APPROVED') {
    $state_title = '✅ Application Approved';
    $state_sub = 'Your application has been approved. You can now access your student dashboard.';
} elseif ($status === 'REJECTED') {
    $state_title = '❌ Application Not Approved';
    $state_sub = 'Your application was not approved at this time. Please contact SDAO for guidance.';
}
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <div class="container">
        ...
        <section class="status-card" aria-labelledby="status-state-title">
            ...
            <div class="status-card__hero">
                ...
                <h2 class="status-card__state-title" id="status-state-title"><?= htmlspecialchars($state_title) ?></h2>
                <p class="status-card__state-sub">
                    <?= htmlspecialchars($state_sub) ?>
                </p>
            </div>
            ...
            <div class="status-card__btn-row">
                ...
                <a href="login.php" class="btn-login" role="button" aria-label="Go to login page">
                    Go to Login
                </a>
            </div>
